% ============================================================
%  VARAKUMAR G  —  Résumé (Sector base: VAPT / Pentest)
...
Security Engineer with 2+ years in \textbf{Vulnerability Assessment and Penetration Testing (VAPT)} across web apps, APIs, networks, \textbf{Active Directory}, and \textbf{OT/ICS}. Independently assessed and exploited a live billing platform end-to-end — \textbf{IDOR}, \textbf{privilege escalation}, \textbf{SQL injection}, \textbf{XSS} — and reported it through a \textbf{GitHub Security Advisory}. Executes AD attack paths (\textbf{Kerberoasting}, \textbf{pass-the-hash}) and validated \textbf{2,800+ CVEs} in OT Security within a year, solo, scoring and prioritizing each with \textbf{CVSS} against \textbf{OWASP Top 10} and \textbf{MITRE ATT\&CK}.
...
\techrow{Security Domains:}{Penetration Testing, Vulnerability Assessment, Active Directory, AppSec \& API Security, ICS/OT Security}
...
\techrow{Vulnerability \& Cloud Scanning:}{Nessus, OpenVAS, Qualys, CloudSploit, NVD, CVSS, AWS, Microsoft Azure, GCP, Oracle Cloud}
...
\begin{itemize}
  \item Reproduced and validated \textbf{OT/SCADA vulnerabilities} against real vendor advisories, growing
        \textbf{CVE coverage from ~400 to 3,200+} within a year — a \textbf{700\% increase} in the company's
        SCADA/OT coverage — scoring each with \textbf{CVSS} and ruling out false positives so effort tracked
        real exploitability.
  \item Assessed OT security products from \textbf{Siemens}, \textbf{Schneider Electric}, \textbf{Rockwell Automation},
        \textbf{ABB}, \textbf{Emerson}, and \textbf{Honeywell}, proving exploit conditions against each vendor's
        protocol stack.
  \item Assessed the \textbf{CloudSploit platform end-to-end} across \textbf{AWS}, \textbf{Azure}, \textbf{GCP}, and
        \textbf{Oracle} against \textbf{CIS Benchmark} controls, contributing reviewed fixes upstream.
  \item Automated security-check development using \textbf{Claude AI}, then walked clients through findings with
        remediation guidance and re-validated fixes.
\end{itemize}
