...
- **Portal-plugin design (approved 2026-07-01, build = follow-up).** Broaden sourcing via
  ...
  - **ATS-API plugins** — one plugin per platform, covers ANY company on it via public JSON
    (fast, reliable, **no browser**): a shared `ATSPlugin` base doing the common fetch/
    normalize, tiny subclasses per platform + company slug. Greenhouse
    (`boards-api.greenhouse.io/v1/boards/<co>/jobs`), Lever (`api.lever.co/v0/postings/<co>`),
    Workday (CxS API), **Eightfold** (Qualcomm), **Zoho Recruit** (Simbian), SmartRecruiters,
    Ashby. Each returns normalized `Job`s with the **job-detail link in `Job.url`** (NOT the
    apply-button link).
  ...
  - **Owner's 7 sites mapped:** Qualcomm→Eightfold, Simbian→Zoho Recruit, cyber-times.in/jobs→
    **OFFLINE** (impersonation probe, skip); Qualys/Sibros/Mattel/EY→detect ATS during build.
  - First follow-up deliverable = the curated portal→ATS→endpoint list, then `ATSPlugin` +
    a couple subclasses, then aggregators.
